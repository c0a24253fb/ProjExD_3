...
class Beam:
    ...
    def __init__(self, bird:"Bird"):
        ...
        """
        self.img = pg.image.load(f"fig/beam.png")  # 画像を読み込みsurfaceを生成
...
class Score:
    ...
    スコア
    """
    def __init__(self, color: tuple[int, int, int], score: int):
...
def main():
    ...
    bird = Bird((300, 200))  #birdインスタンス
    # bomb = Bomb((255, 0, 0), 10)  #bombインスタンス(color, rad)
    # bombs = []
    ...
    while True:
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:
                ...
                beam = Beam(bird)  #beamにbeamインスタンスを生成する。Beamの引数はbirdにする
                beams.append(beam)  # beamsリストにBeamインスタンスを追加          
        screen.blit(bg_img, [0, 0])
        
        for bomb in bombs:  # bombsリストから要素を1つずつ取る
            if bird.rct.colliderect(bomb.rct):
                # ゲームオーバー時に，こうかとん画像を切り替え，1秒間表示させる
                bird.change_img(8, screen)  # こうかとんの画像を8.pngに変更
